...
function WisdomQuotes({ stories, lang, navigate }) {
  ...
  const title = lang === 'vi' ? q.titleVi : q.titleEn
  ...
  return (
    <section className="wisdom-section fade-up">
      ...
      <div className="wisdom-card">
        ...
        <blockquote className="wisdom-text" key={active}>
          {thread}
        </blockquote>
      </div>
      ...
      <div className="wisdom-dots">
        {quotes.map((_, i) => (
          <button
            key={i}
            className={`wisdom-dot ${i === active ? 'active' : ''}`}
            onClick={() => setActive(i)}
